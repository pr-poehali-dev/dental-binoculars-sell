...
const products: Product[] = [
  {
    ...
    price: 119000,
    oldPrice: 140000,
    image: 'https://cdn.poehali.dev/projects/37487b42-26a7-4ea4-bd44-c9a83bc78370/bucket/9121fe26-91b2-4f64-8d42-8733a5b0f057.jpg',
            ...
            {sortedProducts.map((product, index) => (
              <Card key={product.id} className="overflow-hidden hover:shadow-xl transition-shadow duration-300 animate-scale-in product-card-hover relative" style={{ animationDelay: `${index * 100}ms` }}>
                ...
                <div className="aspect-square overflow-hidden bg-gray-100" onClick={() => navigate(`/product/${product.id}`)}>
                  <img src={product.image} alt={product.name} className={`w-full h-full object-cover transition-transform duration-300 ${product.id === 6 ? 'scale-[1.2] hover:scale-[1.35]' : product.id === 5 ? 'scale-[1.7] hover:scale-[1.85]' : product.id === 7 ? 'scale-[1.6] hover:scale-[1.75]' : 'scale-[1.3] hover:scale-[1.45]'}`} />
                </div>
                <CardHeader>
