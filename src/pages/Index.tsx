...
function useConfetti() {
  ...
  const launch = useCallback(() => {
    ...
    for (let i = 0; i < 120; i++) {
      ...
      pieces.push({ x, y: window.innerHeight * 0.3 + Math.random() * 100, vx: (side === 'left' ? 1 : -1) * (2 + Math.random() * 4), vy: -(8 + Math.random() * 8), angle: Math.random() * 360, va: (Math.random() - 0.5) * 10, color: colors[Math.floor(Math.random() * colors.length)], w: 8 + Math.random() * 8, h: 4 + Math.random() * 4, side });
    }
    ...
    function draw(ts: number) {
      ...
      for (const p of pieces) {
        p.x += p.vx; p.y += p.vy; p.vy += 0.25; p.vx *= 0.99; p.angle += p.va;
        ctx.save(); ctx.translate(p.x, p.y); ctx.rotate((p.angle * Math.PI) / 180);
        ctx.fillStyle = p.color; ctx.globalAlpha = Math.max(0, 1 - elapsed / duration);
        ctx.fillRect(-p.w / 2, -p.h / 2, p.w, p.h); ctx.restore();
      }
      if (elapsed < duration) { animFrameRef.current = requestAnimationFrame(draw); }
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <Card className="p-6">
                ...
                <p className="text-gray-400">Более 15 лет в стоматологической сфере. Мы разрабатываем и производим продукт используя высококачественные оптические компоненты / линзы Schott (Германия), Glance (Германия/Корея), HOYA (Япония) и предлагаем только проверенные решения.</p>
              </Card>
              <Card className="p-6">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {sortedProducts.map((product, index) => (
              <Card key={product.id} className="overflow-hidden hover:shadow-xl transition-shadow duration-300 animate-scale-in product-card-hover relative" style={{ animationDelay: `${index * 100}ms` }}>
                ...
                <CardHeader>
                  <CardTitle className="font-display cursor-pointer hover:text-primary transition-colors" onClick={() => navigate(`/product/${product.id}`)}>{product.name}</CardTitle>
                </CardHeader>
                <CardContent>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                ...
                },
                {
                  ...
                  text: 'Осветитель Pro Max — находка для имплантологии. Беспроводной, лёгкий, 35000 ЛК — достаточно для любой операции. Заряда 3х аккумуляторов хватает на весь рабочий день.',
                  rating: 5,
                  product: 'Осветитель Pro Max'
                },
                {
                  name: 'Ольга Игнатьева',
                  role: 'Ортопед, Казань',
                ...
                {
                  name: 'Наталья Байкова',
                  role: 'Детский стоматолог, Новосибирск',
